...
- 异或运算：值相同为0，不相同为1。运算符在`Kotlin`中为`xor`，在`Java`中为`^`，对于`A xor B = C`来说，`A xor C = B`也是成立的。他的性质如下
  - 交换律：`A xor B xor C = A xor C xor B`；
  - 结合律：`(A xor B) xor C = A xor (B xor C)`；
  - 对于任何数，和本身异或的结果是0，`A xor A = 0`，和0异或的结果是本身`A xor 0 = A`，这个性质可以用来查找一些数的遗漏；
  - 自反性：`A xor B xor B = A`，这个其实是上面的性质的实际运用。
...
- 算文字居中的算法
	```kotlin
  height / 2f - (paint.fontMetrics.descent + paint.fontMetrics.ascent) / 2F
  ```